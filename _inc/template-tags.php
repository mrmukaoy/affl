...
if ( ! function_exists( 'affl_posted_by' ) ) :
	/**
	...
	function affl_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $tags_list ) {
				...
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'affl' ) . '</span>', $tags_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
		}
			...
			comments_popup_link(
				sprintf(
					wp_kses(
						...
						__( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'affl' ),
						array(
							'span' => array(
								'class' => array(),
							),
...
if ( ! function_exists( 'affl_post_thumbnail' ) ) :
	/**
...
if ( ! function_exists( 'affl_archive_posted_on' ) ) {
	/**
	 * Prints HTML with meta information for the current post-date/time.
	 */
	function affl_archive_posted_on() {
		$time_string = '<time class="entry-date published updated" datetime="%1$s">%2$s</time>';
		if ( get_the_time( 'U' ) !== get_the_modified_time( 'U' ) ) {
			$time_string = '<time class="entry-date published" datetime="%1$s">%2$s</time><time class="updated" datetime="%3$s">%4$s</time>';
		}
...
if ( ! function_exists( 'affl_archive_posted_by' ) ) {
	/**
	 * Prints HTML with meta information for the current author.
	 */
	function affl_archive_posted_by() {
		$byline = sprintf(
			/* translators: %s: post author. */
...
if ( ! function_exists( 'affl_archive_cats_tags' ) ) {
	/**
	 * Prints HTML with meta information for the categories, tags and comments.
	 */
